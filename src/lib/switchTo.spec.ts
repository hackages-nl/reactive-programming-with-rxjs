...
describe("SwitchTo operator", () => {
  ...
  it("should emit value from the second argument when the first argument emits", () => new Promise((done) => {
    // Given
  ...
  it("should stop emitting data when the subscribe function is called", () => new Promise((done) => {
    // Given
    const head = document.head;
    const event$ = fromEvent(head, "click");
    const interval$ = interval(1000);
